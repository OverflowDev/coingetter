...
function SearchMobile() {
  ...
  return (
    ...
    >
      ...
      <div className="mx-auto dark:text-gray-300 text-dark">
        <button type="submit" className="relative w-max">
          <input type="search"
            ...
            className=" cursor-pointer relative z-10 py-2 w-12 rounded-full border bg-transparent outline-none focus:w-44 focus:cursor-text focus:border-green focus:pl-16 focus:pr-4" 
          />
      ...
      <div className='scrollbar-hide overflow-hidden overflow-y-auto absolute top-11 right-5 w-52 h-24 rounded-lg py-1 dark:bg-gray-300 bg-gray-400 '>
        {filteredData?.slice(0,3).map((coin,i) => {
          return (
            <Link 
              to={`/coin/${coin?.id}`} 
              key={i}
              className='tracking-wider text-dark flex flex-col text-md dark:hover:bg-gray-400 hover:bg-gray-300 rounded px-2'
            >
